...
namespace Lab6
{
    internal class Program
    {
        ...
        {
            //###### Load Periodic Table and Throw an exception ######
            Console.WriteLine("\n###### Load Periodic Table and Throw an exception ######\n");
            PeriodicTable();
            //######################## Item 1 ########################
            Console.WriteLine("\n######################## Item 1 ########################\n");
        ...
        static void ReadAll()
        {
            StreamReader file = new StreamReader("PeriodicTable.json");
            string serialElement = file.ReadLine();

            Console.WriteLine("+------------------------------------------------------+");
            Console.WriteLine("|      Name     | Proton | Neutron |  Weight  | Symbol |");
            Console.WriteLine("+------------------------------------------------------+");
            while (serialElement != null)
            {
                Atom a = JsonSerializer.Deserialize<Atom>(serialElement);
                Console.WriteLine(a);
                serialElement = file.ReadLine();
            }
            Console.WriteLine("+------------------------------------------------------+");
        ...
        static void SerialAndSaveAll()
        {
            string serialElement = "";
            StreamWriter file = new StreamWriter("PeriodicTable.json");

            foreach (Atom a in elements)
            {
                serialElement = JsonSerializer.Serialize(a);
                file.WriteLine(serialElement);
            }
        ...
        static void Read1()
        {
            StreamReader file = new StreamReader("FirstElement.json");
            string serialElement = file.ReadLine();
            Atom a = JsonSerializer.Deserialize<Atom>(serialElement);
            Console.WriteLine("+------------------------------------------------------+");
            Console.WriteLine("|      Name     | Proton | Neutron |  Weight  | Symbol |");
        ...
        static void SerialAndSave1()
        {
            string serialElement = JsonSerializer.Serialize(elements[0]);
            StreamWriter file = new StreamWriter("FirstElement.json");
    ...
    class Atom
    {
        ...
        public static Atom Parse(string objectData)
        {
            ...
            {
                //- use split
                properties = objectData.Split(' ');
                //- Throw exception if not 5 items.
                if (properties.Length != 5)
                {
                    throw new Exception("You have entered invalid data. The atom should be\n" +
                        "described using exactly 5 attributes:\n\n" +
                        "1) Name\n" +
                        "2) Number of Protons\n" +
                        "3) Number of Neutrons\n" +
                        "4) Weight\n" +
                        "5) Symbol.\n\n" +
                        "The attributes must be given in this order, and be\n" +
                        "separated by a single space.\n" +
                        "The number of protons and neutrons must be integers, and\n" +
                        "weight can be any Real Number.\n");
                }
            }
            catch (Exception e)
            {
                Console.WriteLine(e.Message);
                Console.WriteLine("Press any key to continue the program execution.");
                Console.ReadLine();
            }
            //- Initialize the object if no exception occours
            //- Return the object
            return new Atom(properties[0],
        ...
        {
            return $"|{Name,14} |   {Proton,3}  |   {Neutron,3}   | {Weight,8:F4} |   {Symbol,2}   |";
        }
    }
}
